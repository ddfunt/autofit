...
def int_writer(u_A, u_B, u_C, J_min="00", J_max="20", inten="-10.0", Q_rot="300000", freq="25.8", temperature="298",
               flag="default"):  # generates SPCAT input file
    input_file = ""
    # print "freq_max=",freq
    input_file += "Molecule \n"
...
def var_writer(A, B, C, DJ, DJK, DK, dJ, dK, main_flow, flag):  # generates SPCAT input file
    ...
    if main_flow == 'Normal species':
        dA = str(0.02 * float(
            A))  # These are very rough estimates of the uncertainty on the rotational constants.  May need to be considerably refined.
        dB = str(0.01 * float(B))
        dC = str(0.01 * float(C))
        ...
        dA = str(0.005 * float(
            A))  # Assume 1% uncertainties for isotopologues if normal species is known.  Maybe unreasonable.
...
def par_writer_refit(A, B, C, DJ, DJK, DK, dJ, dK):  # generates SPFIT par file
    ...
    constants_to_vary = multchoicebox(
        msg='Choose the distortion constants to vary.  If none are selected, only A, B, and C will be varied in the refined fit.',
        title='Distortion Constant Choice', choices=constant_list)
    ...
    dA = str(0.5 * float(
        A))  # These allow A, B, and C to vary by 50% and the distortions to vary by a factor of 10.  Too much, too little?
    dB = str(0.5 * float(B))
    dC = str(0.5 * float(C))
...
def lin_writer_refit(assignment_list):  # writes a lin file for use with SPFIT
    ...
    for line in assignment_list:
        input_file += line[1][0:2] + ' ' + line[1][2:4] + ' ' + line[1][4:6] + ' ' + \
                      line[2][0:2] + ' ' + line[2][2:4] + ' ' + line[2][4:6] + '                      ' + str(
            line[0]) + ' ' + line[3] + ' 1.0000\n'
...
def cubic_spline(spectrum,
                 new_resolution):  # Cubic spline of spectrum to new_resolution; used pre-peak-picking.  Assumes spectrum is already in order of increasing frequency.
...
def match_to_peaklist(pred_trans, peaklist):
    ...
    threshold = float(enterbox(
        msg="Enter the OMC threshold in MHz.  Predicted transitions that are not at least this close to an experimental peak will not contribute to the fit."))

    for x in range(len(
            pred_trans)):  # matches predicted peaks to peaks in experimental peak list <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        weight = '0.50'
...
def check_bounds(trans_1_center, trans_2_center, trans_3_center, peak_1_uncertainty, peak_2_uncertainty,
                 peak_3_uncertainty, freq_low, freq_high):
    bad_windows = 0
    bad_1 = 0
    bad_2 = 0
    bad_3 = 0
...
def final_uncerts(trans_1_center, trans_2_center, trans_3_center, peak_1_uncertainty, peak_2_uncertainty,
                  peak_3_uncertainty, freq_low, freq_high, full_list, A, B, C, DJ, DJK, DK, dJ, dK, temperature, u_A,
                  u_B, u_C, main_flow, trans_1, trans_2, trans_3):
    (bad_windows, bad_1, bad_2, bad_3) = check_bounds(trans_1_center, trans_2_center, trans_3_center,
    ...
    while bad_windows == 1:
        while bad_1 == -1:
            bad_wind_decision = buttonbox(
                msg='The search window for transition 1 (%s - %s) extends below the range of the spectrum.  Would you like to continue, try a new uncertainty for this transition, or choose a new transition?' % (
                trans_1[2], trans_1[3]), choices=('Continue', 'New Uncertainty', 'Choose New Transition'))
            if bad_wind_decision == 'Continue':
                bad_windows = 0
                bad_1 = 0
            elif bad_wind_decision == 'New Uncertainty':
                peak_1_uncertainty = float(enterbox(
                    msg="Enter the frequency uncertainty for transition 1 (%s - %s) in MHz.  Its estimated position is %s and its estimated uncertainty is %s MHz." % (
                    trans_1[2], trans_1[3], trans_1_center, trans_1_uncert)))
                (bad_windows, bad_1, bad_2, bad_3) = check_bounds(trans_1_center, trans_2_center, trans_3_center,
                                                                  peak_1_uncertainty, peak_2_uncertainty,
                                                                  peak_3_uncertainty, freq_low, freq_high)
        ...
        while bad_2 == -1:
            bad_wind_decision = buttonbox(
                msg='The search window for transition 2 (%s - %s) extends below the range of the spectrum.  Would you like to continue, try a new uncertainty for this transition, or choose a new transition?' % (
                trans_2[2], trans_2[3]), choices=('Continue', 'New Uncertainty', 'Choose New Transition'))
            if bad_wind_decision == 'Continue':
                bad_windows = 0
                bad_2 = 0
            elif bad_wind_decision == 'New Uncertainty':
                peak_2_uncertainty = float(enterbox(
                    msg="Enter the frequency uncertainty for transition 2 (%s - %s) in MHz.  Its estimated position is %s and its uncertainty is %s MHz." % (
                    trans_2[2], trans_2[3], trans_2_center, trans_2_uncert)))
                (bad_windows, bad_1, bad_2, bad_3) = check_bounds(trans_1_center, trans_2_center, trans_3_center,
                                                                  peak_1_uncertainty, peak_2_uncertainty,
                                                                  peak_3_uncertainty, freq_low, freq_high)
            elif bad_wind_decision == 'Choose New Transition':
                ...
                peak_2_uncertainty = float(enterbox(
                    msg="Enter the frequency uncertainty for transition 2 (%s - %s) in MHz.  Its estimated position is %s and its estimated uncertainty is %s MHz." % (
                    trans_2[2], trans_2[3], trans_2_center, trans_2_uncert)))
                (bad_windows, bad_1, bad_2, bad_3) = check_bounds(trans_1_center, trans_2_center, trans_3_center,
                                                                  peak_1_uncertainty, peak_2_uncertainty,
                                                                  peak_3_uncertainty, freq_low, freq_high)
        ...
        while bad_3 == -1:
            bad_wind_decision = buttonbox(
                msg='The search window for transition 3 (%s - %s) extends below the range of the spectrum.  Would you like to continue, try a new uncertainty for this transition, or choose a new transition?' % (
                trans_3[2], trans_3[3]), choices=('Continue', 'New Uncertainty', 'Choose New Transition'))
            if bad_wind_decision == 'Continue':
                bad_windows = 0
                bad_3 = 0
            elif bad_wind_decision == 'New Uncertainty':
                peak_3_uncertainty = float(enterbox(
                    msg="Enter the frequency uncertainty for transition 3 (%s - %s) in MHz.  Its estimated position is %s and its uncertainty is %s MHz." % (
                    trans_3[2], trans_3[3], trans_3_center, trans_3_uncert)))
                (bad_windows, bad_1, bad_2, bad_3) = check_bounds(trans_1_center, trans_2_center, trans_3_center,
                                                                  peak_1_uncertainty, peak_2_uncertainty,
                                                                  peak_3_uncertainty, freq_low, freq_high)
            elif bad_wind_decision == 'Choose New Transition':
                ...
                peak_3_uncertainty = float(enterbox(
                    msg="Enter the frequency uncertainty for transition 3 (%s - %s) in MHz.  Its estimated position is %s and its estimated uncertainty is %s MHz." % (
                    trans_3[2], trans_3[3], trans_3_center, trans_3_uncert)))
                (bad_windows, bad_1, bad_2, bad_3) = check_bounds(trans_1_center, trans_2_center, trans_3_center,
                                                                  peak_1_uncertainty, peak_2_uncertainty,
                                                                  peak_3_uncertainty, freq_low, freq_high)
        ...
        while bad_1 == 1:
            bad_wind_decision = buttonbox(
                msg='The search window for transition 1 (%s - %s) extends above the range of the spectrum.  Would you like to continue, try a new uncertainty for this transition, or choose a new transition?' % (
                trans_1[2], trans_1[3]), choices=('Continue', 'New Uncertainty', 'Choose New Transition'))
            if bad_wind_decision == 'Continue':
                bad_windows = 0
                bad_1 = 0
                ...
                peak_1_uncertainty = float(enterbox(
                    msg="Enter the frequency uncertainty for transition 1 (%s - %s) in MHz.  Its estimated position is %s and its uncertainty is %s MHz." % (
                    trans_1[2], trans_1[3], trans_1_center, trans_1_uncert)))
                (bad_windows, bad_1, bad_2, bad_3) = check_bounds(trans_1_center, trans_2_center, trans_3_center,
                                                                  peak_1_uncertainty, peak_2_uncertainty,
                                                                  peak_3_uncertainty, freq_low, freq_high)
        ...
        while bad_2 == 1:
            bad_wind_decision = buttonbox(
                msg='The search window for transition 2 (%s - %s) extends above the range of the spectrum.  Would you like to continue, try a new uncertainty for this transition, or choose a new transition?' % (
                trans_2[2], trans_2[3]), choices=('Continue', 'New Uncertainty', 'Choose New Transition'))
            if bad_wind_decision == 'Continue':
                bad_windows = 0
                bad_2 = 0
        ...
        while bad_3 == 1:
            bad_wind_decision = buttonbox(
                msg='The search window for transition 3 (%s - %s) extends above the range of the spectrum.  Would you like to continue, try a new uncertainty for this transition, or choose a new transition?' % (
                trans_3[2], trans_3[3]), choices=('Continue', 'New Uncertainty', 'Choose New Transition'))
            if bad_wind_decision == 'Continue':
                bad_windows = 0
                bad_3 = 0
        ...
        if (trans_1 == trans_2) or (trans_2 == trans_3) or (trans_1 == trans_3):
            same_trans_decision = buttonbox(
                msg='You do not have three distinct transitions.  Choose new ones manually, or quit?',
                choices=('Choose manually', 'Quit'))
            if same_trans_decision == 'Quit':
                quit()
            elif same_trans_decision == 'Choose manually':
                (highest_uncert, trans_1, trans_2, trans_3) = triple_selection(full_list, A, B, C, DJ, DJK, DK, dJ, dK,
                                                                               temperature, freq_high, freq_low, u_A,
                                                                               u_B, u_C, main_flow, flag="force manual")
                trans_1_uncert = float(trans_1[4])
                trans_2_uncert = float(trans_2[4])
                trans_3_uncert = float(trans_3[4])
                peak_1_uncertainty = float(enterbox(
...
def triples_gen(window_decision, trans_1_uncert, trans_2_uncert, trans_3_uncert, freq_uncertainty, peak_1_uncertainty,
                peak_2_uncertainty, peak_3_uncertainty, peaklist, freq_low, freq_high, isotopomer_count, decision,
                full_list, A, B, C, DJ, DJK, DK, dJ, dK, temperature, u_A, u_B, u_C, main_flow, trans_1, trans_2,
                trans_3, scale_factor):
    user_flag = 0
    est_unc_flag = 0
    same_flag = 0
    ...
    while trans_1_peaks == [] or trans_2_peaks == [] or trans_3_peaks == []:  # this loops until there are peaks around each member of the triple
        ...
        while uncertainty_flag == 1:
            ...
            if freq_uncertainty == 0.0 and user_flag == 1 and same_flag == 1 and isotopomer_count == 0:
                freq_uncertainty = float(enterbox(
                    msg="Enter the frequency uncertainty in MHz.  The largest uncertainty from your fitting peaks is %s MHz." % (
                    highest_uncert)))
            ...
            (trans_1, trans_2, trans_3, peak_1_uncertainty, peak_2_uncertainty, peak_3_uncertainty) = final_uncerts(
                trans_1_center, trans_2_center, trans_3_center, peak_1_uncertainty, peak_2_uncertainty,
                peak_3_uncertainty, freq_low, freq_high, full_list, A, B, C, DJ, DJK, DK, dJ, dK, temperature, u_A, u_B,
                u_C, main_flow, trans_1, trans_2, trans_3)
            trans_1_center = float(trans_1[1])
            trans_2_center = float(trans_2[1])
            trans_3_center = float(trans_3[1])
            ...
            for freq_p, inten_p in peaklist:
                ...
                if abs(float(trans_2_center) - float(
                        freq_p)) < peak_2_uncertainty:  # this bit finds peaks in the real spectrum that are near the predicted peaks
            ...
            num_of_triples = len(trans_1_peaks) * len(trans_2_peaks) * len(
                trans_3_peaks)  # this tells you how many entries there will be in the all_combo_list
            ...
            if isotopomer_count == 0:
                decision = buttonbox(
                    msg='There are %s triples in this calculation, which will take roughly %s. Would you like to continue, try new uncertainty, or quit?' % (
                    str(num_of_triples), time_string), choices=('Continue', 'Quit', 'new uncertainty'))
...
def refine_fits(job_name, isotope_ID, u_A, u_B, u_C, Jmax, freq_high, temperature, fits, DJ, DJK, DK, dJ, dK, peaklist,
                main_flow):
    ...
    while all_fitting_done == 0:
        ...
        fit_peaklist = list(OrderedDict.fromkeys(
            fit_peaklist))  # This removes duplicates if some transitions are both check and fit transitions, which might bias SPFIT towards hitting those in particular.
        ...
        more_transitions_decision = buttonbox(msg='Would you like to add additional transitions into the expanded fit?',
                                              choices=('Yes', 'No'))
        ...
        if add_more_transitions == 1:
            extra_peaks = multchoicebox(
                msg='Choose the transitions to use in the new fit.  Previous fitting and scoring transitions will also be included.  Duplicates will be removed automatically.',
                title='Result Sorting Setup', choices=cat_reader(freq_high, freq_low, flag="refit"))
            extra_peaks_clean = []
        ...
        updated_trans = trans_freq_refit_reader(
            fit_peaklist)  # Finds updated predicted frequencies with improved A, B, and C estimates.
        ...
        while fitting_done == 0:
            ...
            best_matches = match_to_peaklist(updated_trans,
                                             peaklist)  # Assigns closest experimental peak frequencies to transitions
            ...
            fit_decision = buttonbox(
                msg='Fitting has finished. Would you like to accept the current fit or try again while changing inclusion threshold or allowing different distortions to vary?',
                choices=('Accept Current Fit', 'Try Again'))
        ...
        all_fit_decision = buttonbox(
            msg='Would you like to refine another previous result or are you done with this species?',
            choices=('Refine Another Result', 'Done with this one!'))
...
def triple_selection(full_list, A, B, C, DJ, DJK, DK, dJ, dK, temperature, freq_high, freq_low, u_A, u_B, u_C,
                     main_flow, flag):
    if flag == "undecided":
        triple_style = buttonbox(
            msg='How do you want to choose fitting transitions?  You can choose from an automatically scored list, or select three transitions manually.',
            choices=('Automatic scoring', 'Manual selection'))
    ...
    if triple_style == 'Automatic scoring':
        ...
        for i in range(0, total_check_num - 2):
            for j in range(i + 1, total_check_num - 1):
                for k in range(j + 1, total_check_num):
                    ...
                    trans_3 = full_list[k]
                    dependence = abs(
                        dependence_test(float(A), float(B), float(C), float(DJ), float(DJK), float(DK), float(dJ),
                                        float(dK), trans_1, trans_2, trans_3, temperature, freq_high, freq_low, u_A,
                                        u_B, u_C, main_flow))
                    ...
                    intensity_avg = abs(float(trans_1[0]) + float(trans_2[0]) + float(trans_3[
                                                                                          0])) / 3  # For all sane T and dipoles, intensities are negative, so the greatest sum of abs(intensity) is the smallest set of peaks.
                    ...
                    if (abs(float(trans_3[1]) - freq_low) <= 100) or (abs(float(trans_3[1]) - freq_high) <= 100):
                        boundary_penalty += 1
                    triples_scores.append(
                        (i, j, k, dependence, RMS_function, intensity_avg, worst_RMS, boundary_penalty))
        ...
        for entry in triples_scores:
            scaled_dep = entry[
                             3] / max_dependence  # big dependence is good and should help the score, big values of RMS and intensity are bad and should hurt the score.
            scaled_RMS = 1 - entry[4] / max_RMS
            scaled_inten = 1 - entry[5] / max_intensity
            boundary_penalty = 1 / (1 + entry[7])
            scaled_score = (
                           45 * scaled_dep + 45 * scaled_RMS + 10 * scaled_inten) * boundary_penalty  # Reduces score based on how many transitions are close to a spectrum edge.
            stupid_easyGUI_hack = 1 - (
            scaled_score / 100)  # EasyGUI does a sort before displaying choices, presenting them from least to greatest.  This is a dumb way to get it to display triples choices in the order that I want it to, from best to worst.
            scaled_triples_scores.append((entry[0], entry[1], entry[2], entry[3], entry[4], entry[5], scaled_dep,
                                          scaled_RMS, scaled_inten, scaled_score, stupid_easyGUI_hack, entry[6]))
        ...
        for entry in scaled_triples_scores:
            ...
            triples_choice_list.append((entry[10], entry[9], entry[11], trans_1[1], trans_1[2], trans_1[3], trans_2[1],
                                        trans_2[2], trans_2[3], trans_3[1], trans_3[2], trans_3[3], trans_1[4],
                                        trans_2[4], trans_3[4], entry[0], entry[1], entry[2]))
        ...
        while user_satisfied == 0:
            ...
            if len(choice_clean) >= 3:
                ...
                dependence = dependence_test(float(A), float(B), float(C), float(DJ), float(DJK), float(DK), float(dJ),
                                             float(dK), trans_1, trans_2, trans_3, temperature, freq_high, freq_low,
                                             u_A, u_B, u_C, main_flow)

                decision1 = buttonbox(
                    msg='These three peaks have a linear dependence of %s and the highest uncertainty in MHz is %s. A linear dependence between -1.0 and 1.0 is probably too low. Would you like to continue, or try three new peaks?' % (
                    str(dependence), highest_uncert), choices=('Continue', 'Try three new peaks'))
...
def dependence_test(A, B, C, DJ, DJK, DK, dJ, dK, trans_1, trans_2, trans_3, T, freq_high, freq_low, u_A, u_B, u_C,
                    main_flow):
    int_writer(u_A, u_B, u_C, J_min="00", J_max="20", inten="-10.0", Q_rot="300000", freq="100.0", temperature=T,
               flag="default")
    ...
    var_writer(A, B, C, DJ, DJK, DK, dJ, dK, main_flow,
               flag="uncert")  # This re-runs SPCAT at the initial constants so that other things that read from default.cat are correct after this function is executed.
    run_SPCAT()
...
def read_coords_file():
    ...
    for entry in an:  # changes atomic numbers to masses of NS; these are all the atoms in molecules I've taken spectra of.  I need to take data on more types of atoms!
        ...
        if entry == '9' or entry == 'F':
            mass.append(
                18.998403)  # No "has_F_flag" because only one stable isotope.  Change if you are working with molecules from an 18F production facility.
...
def isotopomers(expt_A, expt_B,
                expt_C):  # Uses a file of coordinates (Gaussian output format) + expt'l NS constants to generate A,B,C values of isotopomers
    ...
    atoms_to_vary = multchoicebox(msg='Choose atoms to isotopically substitute', title='Isotope Substitution',
                                  choices=atom_list)
...
def fit_triples(list_a, list_b, list_c, trans_1, trans_2, trans_3, top_17, peaklist, file_num, A, B, C, DJ, DJK, DK, dJ,
                dK):
    ...
    pred_ratio = pow(10, max(float(trans_1[0]), float(trans_2[0]), float(trans_3[0])) - min(float(trans_1[0]),
                                                                                            float(trans_2[0]),
                                                                                            float(trans_3[0])))
    ...
    for freq_1, inten_1, diff_1 in temp_list_a:
        for freq_2, inten_2, diff_2 in temp_list_b:
            for freq_3, inten_3, diff_3 in temp_list_c:
                real_ratio = max(float(inten_1), float(inten_2), float(inten_3)) / min(float(inten_1), float(inten_2),
                                                                                       float(inten_3))
                avg_diff = (diff_1 + diff_2 + diff_3) / 3
                scaled_diff = avg_diff * (abs(
                    real_ratio - pred_ratio) + 1)  # Freq. difference scaled by deviation of intensity ratio from predicted
    ...
    for freq_1, inten_1, freq_2, inten_2, freq_3, inten_3, total_diff in sorted_full_list:
        all_combo_list_file.write(
            str(freq_1) + "," + str(inten_1) + "," + str(freq_2) + "," + str(inten_2) + "," + str(freq_3) + "," + str(
                inten_3) + ", \n")
    ...
    for all_combo_line in all_combo_list_file:
        ...
        peaks_triple = [(all_combo_line[0], all_combo_line[1]), (all_combo_line[2], all_combo_line[3]),
                        (all_combo_line[4], all_combo_line[5])]
        ...
        input_file += trans_1[2][0:2] + ' ' + trans_1[2][2:4] + ' ' + trans_1[2][4:6] + ' ' + \
                      trans_1[3][0:2] + ' ' + trans_1[3][2:4] + ' ' + trans_1[3][4:6] + '                      ' + \
                      peaks_triple[0][0] + ' 0.50 1.0000\n'
        input_file += trans_2[2][0:2] + ' ' + trans_2[2][2:4] + ' ' + trans_2[2][4:6] + ' ' + \
                      trans_2[3][0:2] + ' ' + trans_2[3][2:4] + ' ' + trans_2[3][4:6] + '                      ' + \
                      peaks_triple[1][0] + ' 0.50 1.0000\n'
        input_file += trans_3[2][0:2] + ' ' + trans_3[2][2:4] + ' ' + trans_3[2][4:6] + ' ' + \
                      trans_3[3][0:2] + ' ' + trans_3[3][2:4] + ' ' + trans_3[3][4:6] + '                      ' + \
                      peaks_triple[2][0] + ' 0.50 1.0000\n'
        counter = 0
        for line in top_17:  # the hack that adds in the check transitions but doesn't use them in the fit
            input_file += line[2][0:2] + ' ' + line[2][2:4] + ' ' + line[2][4:6] + ' ' + \
                          line[3][0:2] + ' ' + line[3][2:4] + ' ' + line[3][4:6] + '                      ' + '%s.0' % (
            str(counter)) + ' 0.00 1.0000\n'
            counter += 1
        ...
        for x in range(len(file_list)):
            if file_list[-x][11:14] == "RMS":
                rms_fit = float(file_list[-x][
                                22:32])  # note - assumes RMS fit error is less than 1 GHz.  Change 22 to 21 if this is a problem.
        ...
        for x in range(len(
                top_17)):  # matches peaks in the top 17 to peaks in experimental peak list <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            qnum_up = top_17[x][2]
            qnum_low = top_17[x][3]
            real_omc = 1.0
            ...
            if real_omc == 1.0:
                real_omc = omc_low
            omc_list.append((omc_low, real_omc,
                             omc_inten))  # current_peak,qnum_up,qnum_low)) you can add in this extra output, but its slower
        ...
        score = str(len([omc for omc in omc_avg if
                         omc < 2.0]))  # scores the accuracy of the fit, currently based on a peak being within 2 MHz which may be too coarse
        ...
        if (omc_inten_unitless_stdev <= 0.5 * theor_inten_unitless_stdev) or (
            omc_inten_unitless_stdev >= 1.5 * theor_inten_unitless_stdev):
            score_inten_penalty += 1
        ...
        if float(A_1) >= float(B_1) and float(B_1) >= float(C_1) and float(C_1) > 0:
            if int(score) < 10:  # makes sorting work properly later
                score = '0' + score
            output_file += 'score = ' + ' ' + score + ' ' + "Const = " + str(A_1) + ' ' + str(B_1) + ' ' + str(
                C_1) + ' ' + "average omc = " + str(avg) + '  ' + "avg w/out peaks over edge = " + str(
                real_avg) + ' ' + "avg w/ inten penalty = " + str(penalized_avg) + "\n"

            if real_avg <= 0.2:  # appends good finds (RMS < 0.2 MHz, ignoring peaks over edge) to interim file for each processor
                interim_output = 'score = ' + ' ' + score + ' ' + "Const = " + str(A_1) + ' ' + str(B_1) + ' ' + str(
                    C_1) + ' ' + "average omc = " + str(avg) + '  ' + "avg w/out peaks over edge = " + str(
                    real_avg) + ' ' + "avg w/ inten penalty = " + str(penalized_avg) + "\n"
    ...
    fh_final.close()
    os.system(
        "sort -r 'final_output%s.txt'>sorted_final_out%s.txt" % (str(file_num), str(file_num)))  # sorts output by score
...
if __name__ == '__main__':  # multiprocessing imports script as module
    ...
    main_flow = buttonbox(
        msg='Are you searching for a normal species spectrum or singly-substituted isotopologues (already have NS experimental constants)?',
        choices=('Normal species', 'Isotopologues'))
    ...
    while file_flag == 1:
        filename = multenterbox(
            "Choose a folder name for the calculation. If the folder already exists, this dialogue will reset.", "",
            ["Job Name"])
        job_name = filename[0]
        marker1 = 0
        for file1 in x:
    ...
    if file_decision_flag == 0:
        ...
        fieldNames = ["a dipole (%s)" % (u_A), "b dipole (%s)" % (u_B), "c dipole (%s)" % (u_C), "A (%s MHz)" % (A),
                      "B (%s MHz)" % (B), "C (%s MHz)" % (C), "-DJ (%s MHz)" % (DJ), "-DJK (%s MHz)" % (DJK),
                      "-DK (%s MHz)" % (DK), "-dJ (%s MHz)" % (dJ), "-dK (%s MHz)" % (dK), \
                      "number of processors (%s)" % (processors),
                      "upper intensity limit on exp spectrum (%s)" % (inten_high),
                      "lower intensity limit on exp spectrum(%s)" % (inten_low), "temperature (%sK)" % (temperature),
                      "Jmax (%s)" % (Jmax)]
    ...
    fh = numpy.loadtxt(fileopenbox(
        msg="Enter the spectrum file in two column format: frequency intensity"))  # loads full experimental data file, not just list of peaks

    spectrum_2kHz = cubic_spline(fh,
                                 0.002)  # Interpolates experimental spectrum to a 2 kHz resolution with a cubic spline.  Gives better peak-pick values.
    (peaklist, freq_low, freq_high) = peakpicker(spectrum_2kHz, inten_low,
                                                 inten_high)  # Calls slightly modified version of Cristobal's routine to pick peaks instead of forcing user to do so.
    ...
    if main_flow == 'Isotopologues':
        filter_level = float(enterbox(
            msg="Isotopologue search transitions are filtered to only include strong NS transitions in the experimental spectrum.  Enter a scale factor (relative to the lower intensity threshold) to use in this filtering.  Enter zero if NS lines are not present."))
        full_list = intensity_filter(full_list, peaklist, inten_low, filter_level)

    (highest_uncert, trans_1, trans_2, trans_3) = triple_selection(full_list, A, B, C, DJ, DJK, DK, dJ, dK, temperature,
                                                                   freq_high, freq_low, u_A, u_B, u_C, main_flow,
                                                                   flag="undecided")
    ...
    if main_flow == 'Normal species':
        ...
        window_decision = buttonbox(msg='What method would you prefer to use for determining the uncertainty window?',
                                    choices=('User-defined, same for each', 'User-defined, different for each',
        ...
        (trans_1, trans_2, trans_3, trans_1_peaks, trans_2_peaks, trans_3_peaks, peak_1_uncertainty, peak_2_uncertainty,
         peak_3_uncertainty, num_of_triples, decision) = triples_gen(window_decision, trans_1_uncert, trans_2_uncert,
                                                                     trans_3_uncert, freq_uncertainty,
                                                                     peak_1_uncertainty, peak_2_uncertainty,
                                                                     peak_3_uncertainty, peaklist, freq_low, freq_high,
                                                                     isotopomer_count, decision, full_list, A, B, C, DJ,
                                                                     DJK, DK, dJ, dK, temperature, u_A, u_B, u_C,
                                                                     main_flow, trans_1, trans_2, trans_3, scale_factor)
        ...
        if check_peaks_list == []:
            ...
            choice_method = buttonbox(msg='Choose the method for scoring the results:', title='Scoring Options',
                                      choices= \
                                          ('Use the 10 most intense peaks (default)',
                                           'Choose an arbitrary set of peaks from the prediction'))
            if choice_method == 'Use the 10 most intense peaks (default)':
                top_peaks = cat_reader(freq_high, freq_low, flag="default")[
                            0:10]  # grab the most intense peaks from the predicted spectrum
            else:
                top_peaks = multchoicebox(msg='Pick the transitions that you want to use for sorting the results',
                                          title='Result Sorting Setup',
                                          choices=cat_reader(freq_high, freq_low, flag="default"))
                top_peaks_clean = []
        ...
        top_peaks_3cut = []
        for entry in top_peaks:
            if (entry[2] == trans_1[2] and entry[3] == trans_1[3]) or (
                    entry[2] == trans_2[2] and entry[3] == trans_2[3]) or (
                    entry[2] == trans_3[2] and entry[3] == trans_3[3]):
                pass
            else:
    ...
    C: %s \n DJ: %s \n DJK: %s \n DK: %s \n dJ: %s \n dK: %s \n processors: %s \n freq_high: %s \n freq_low: %s \n \
    inten_high: %s \n inten_low: %s \n Temp: %s \n Jmax: %s \n freq_uncertainty: %s \n number of triples: %s \n Check peaks:\n%s \n trans_1: %s \n trans_2: %s \n trans_3: %s " % (
        job_name, u_A, u_B, u_C, A, B, C, DJ, DJK, DK, dJ, dK, str(processors), str(freq_high), \
        str(freq_low), str(inten_high), str(inten_low), str(temperature), str(Jmax), str(freq_uncertainty),
        str(num_of_triples), fitting_peaks_str, str(trans_1), str(trans_2), str(trans_3))
        ...
        new_list = [(len(trans_1_peaks), "trans_1_peaks"), (len(trans_2_peaks), "trans_2_peaks"),
                    (len(trans_3_peaks), "trans_3_peaks")]
        new_list.sort()
        ...
        random.shuffle(
            list_c_peaks)  # Shuffle so that each processor gets a range of values for the third peak, not processor 0 getting only the lowest frequencies.
        ...
        for num in range(processors):
            ...
            vars()["p%s" % str(num)] = Process(target=fit_triples, args=(
            trans_x_peaks, trans_y_peaks, trans_z_peaks, trans_1, trans_2, trans_3, top_peaks_3cut, peaklist, num, A, B,
            C, DJ, DJK, DK, dJ, dK))
        ...
        refine_fit_decision = buttonbox(
            msg='Would you like to further refine any of the recent results by allowing distortions to vary?',
        ...
        if refine_fit_decision == "No, I'm done!":
            ...
            refine_fits(job_name, isotope_ID, u_A, u_B, u_C, Jmax, freq_high, temperature, fits, DJ, DJK, DK, dJ, dK,
                        peaklist, main_flow)

        continue_decision = buttonbox(msg='Would you like to perform a search for singly-substituted isotopologues?',
                                      choices=('Yes', 'No'))
        if continue_decision == 'No':
            ...
            (highest_uncert, trans_1, trans_2, trans_3) = triple_selection(full_list, A, B, C, DJ, DJK, DK, dJ, dK,
                                                                           temperature, freq_high, freq_low, u_A, u_B,
                                                                           u_C, main_flow, flag="undecided")
    ...
    if main_flow == 'Isotopologues':
        ...
        for isotopologue in calc_isotopomer_ABC:
            ...
            for number in range(processors):
                y = subprocess.Popen("cp SPFIT%s.EXE %s\SPFIT%s.EXE" % (number, isotope_ID, number),
            ...
            if (trans_1_center == 0) or (trans_2_center == 0) or (trans_3_center == 0):
                bad_triples = buttonbox(
                    msg='One of your fitting transitions is no longer within bounds after isotopologue scaling.  Please manually choose new transitions or quit.',
                    choices=('Choose new transitions', 'Quit'))
                if bad_triples == 'Quit':
                    quit()
                elif bad_triples == 'Choose new transitions':
                    (highest_uncert, trans_1, trans_2, trans_3) = triple_selection(full_list, A, B, C, DJ, DJK, DK, dJ,
                                                                                   dK, temperature, freq_high, freq_low,
                                                                                   u_A, u_B, u_C, main_flow,
                                                                                   flag="force manual")
                    trans_1_uncert = float(trans_1[4])
                    trans_2_uncert = float(trans_2[4])
            ...
            if isotopomer_count == 0:
                window_decision = buttonbox(
                    msg='What method would you prefer to use for determining the uncertainty window?', choices=(
                    'User-defined, same for each', 'User-defined, different for each', 'Three times SPCAT uncertainty'))
                peak_1_uncertainty = 0
                peak_2_uncertainty = 0
                peak_3_uncertainty = 0
                decision = ""

            (trans_1, trans_2, trans_3, trans_1_peaks, trans_2_peaks, trans_3_peaks, peak_1_uncertainty,
             peak_2_uncertainty, peak_3_uncertainty, num_of_triples, decision) = triples_gen(window_decision,
                                                                                             trans_1_uncert,
                                                                                             trans_2_uncert,
                                                                                             trans_3_uncert,
                                                                                             freq_uncertainty,
                                                                                             ...
                                                                                             peaklist, freq_low,
                                                                                             freq_high,
                                                                                             isotopomer_count, decision,
                                                                                             full_list, A, B, C, DJ,
                                                                                             DJK, DK, dJ, dK,
                                                                                             temperature, u_A, u_B, u_C,
                                                                                             main_flow, trans_1,
                                                                                             trans_2, trans_3,
                                                                                             scale_factor)
            ...
            if check_peaks_list == []:
                ...
                if choice_method == 'Use the 10 most intense peaks (default)':
                    ...
                    if isotopomer_count == 0:
                        ...
                        top_peaks = multchoicebox(
                            msg='Pick the transitions that you want to use for sorting the results',
                            title='Result Sorting Setup', choices=temp_top_peaks)
                        top_peaks_clean = []
                        for entry in top_peaks:
                            clean = entry[2:43]
                            re_split = clean.split("', '")
...
C: %s \n DJ: %s \n DJK: %s \n DK: %s \n dJ: %s \n dK: %s \n processors: %s \n freq_high: %s \n freq_low: %s \n \
inten_high: %s \n inten_low: %s \n Temp: %s \n Jmax: %s \n freq_uncertainty: %s \n number of triples: %s \n Check peaks:\n%s \n trans_1: %s \n trans_2: %s \n trans_3: %s " % (
            job_name, isotope_ID, u_A, u_B, u_C, curr_A, curr_B, curr_C, DJ, DJK, DK, dJ, dK, str(processors),
            str(freq_high), \
            str(freq_low), str(inten_high), str(inten_low), str(temperature), str(Jmax), str(freq_uncertainty),
            str(num_of_triples), fitting_peaks_str, str(trans_1), str(trans_2), str(trans_3))
            ...
            for num in range(processors):
                ...
                vars()["p%s" % str(num)] = Process(target=fit_triples, args=(
                trans_x_peaks, trans_y_peaks, trans_z_peaks, trans_1, trans_2, trans_3, top_peaks_3cut, peaklist, num,
                A, B, C, DJ, DJK, DK, dJ, dK))
            ...
            a = subprocess.Popen(
                'cat sorted_final_out*.txt |sort -t "=" -k 4 -n > sorted_omc_cat_%s.txt' % (isotope_ID), shell=True)
            a.wait()

            a = subprocess.Popen(
                'cat sorted_final_out*.txt |sort -t "=" -k 6 -n > sorted_inten_omc_cat_%s.txt' % (isotope_ID),
                shell=True)
            a.wait()
        ...
        for isotopologue in calc_isotopomer_ABC:
            ...
            codebox(
                msg='Fitting routine has finished.  These are the best 100 results for %s, saved in best100_%s.txt.' % (
                isotope_ID, isotope_ID), text=fits)

            refine_fit_decision = buttonbox(
                msg='Would you like to further refine any of the recent results by allowing distortions to vary?',
                choices=('Yes!', "No, I'm done with this isotopologue!"))
            if refine_fit_decision == "No, I'm done with this isotopologue!":
                pass
            else:
                refine_fits(job_name, isotope_ID, u_A, u_B, u_C, Jmax, freq_high, temperature, fits, DJ, DJK, DK, dJ,
                            dK, peaklist, main_flow)
